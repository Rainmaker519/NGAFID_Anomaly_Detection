...
class RLSTMCell(tf.compat.v1.nn.rnn_cell.BasicLSTMCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, state_is_tuple=True, activation=None, reuse=None, dense=None):
        ...
        """
        super(tf.compat.v1.nn.rnn_cell.BasicLSTMCell, self).__init__(_reuse=reuse)
...
class RSLSTMCell(tf.compat.v1.nn.rnn_cell.BasicLSTMCell):
    """Basic LSTM recurrent network cell.
...
class RKLSTMCell(tf.compat.v1.nn.rnn_cell.BasicLSTMCell):
    """Basic LSTM recurrent network cell.
...
class RGRUCell(tf.compat.v1.nn.rnn_cell.GRUCell):
    ...
    def __init__(self, num_units, activation=None, reuse=None, kernel_initializer=None, bias_initializer=None):
        super(tf.compat.v1.nn.rnn_cell.GRUCell, self).__init__(_reuse=reuse)
        self._num_units = num_units
        self._activation = activation or tf.nn.tanh
...
def Model(_j, _abnormal_data, _abnormal_label, _hidden_num, _elem_num, _file_name, _partition):
    ...
    with g.as_default():
        ...
        # create RNN cell
        if cell_type == 0:
            enc_cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(_hidden_num)
            dec_cell = tf.compat.v1.nn.rnn_cell.BasicRNNCell(_hidden_num)
        if cell_type == 1:
        ...
        with tf.variable_scope('decoder') as vs:
            ...
            if decode_without_input:
                ...
                if _j == 0:
                    (dec_outputs, dec_state_) = tf.compat.v1.nn.dynamic_rnn(pure_dec_cell, dec_input, initial_state=enc_state,
                                                                  dtype=tf.float32)
                elif _j == 1:
                    (dec_outputs, dec_state_) = tf.compat.v1.nn.dynamic_rnn(residual_dec_cell, dec_input, initial_state=enc_state,
                                                                  dtype=tf.float32)
                else:
                    (dec_outputs, dec_state_) = tf.compat.v1.nn.dynamic_rnn(dec_cell, dec_input, initial_state=enc_state,
                                                                  dtype=tf.float32)
                ...
                inference_helper = tfa.seq2seq.InferenceHelper(
                ...
                if _j == 0:
                    inference_decoder = tfa.seq2seq.BasicDecoder(pure_dec_cell, inference_helper, enc_state,
                                                                        output_layer=projection_layer)
                elif _j == 1:
                    inference_decoder = tfa.seq2seq.BasicDecoder(residual_dec_cell, inference_helper, enc_state,
                                                                        output_layer=projection_layer)
                else:
                    inference_decoder = tfa.seq2seq.BasicDecoder(dec_cell, inference_helper, enc_state,
                                                                        output_layer=projection_layer)

                dec_outputs, _, _ = tfa.seq2seq.dynamic_decode(inference_decoder, impute_finished=True,
...
def RunModel(_abnormal_data, _abnormal_label, _hidden_num, _elem_num, _file_name, _partition):
    ...
    for j in range(ensemble_space):
        graph, p_input, dec_outputs, loss, optimizer, saver = Model(j, _abnormal_data, _abnormal_label, _hidden_num,
                                                                    _elem_num, _file_name, _partition)
        ...
        with tf.Session(graph=graph, config=config) as sess:
            ...
            if save_model:
                save_path = saver.save(sess, './saved_model/' + pathlib.Path(file_name).parts[
                    0] + '/masked_skip_rnn_seq2seq_' + str(cell_type) + '_' + os.path.basename(file_name) + '.ckpt')
    ...
    if not partition:
        ...
        np.savetxt(
            './saved_result/' + pathlib.Path(file_name).parts[0] + '/masked_skip_rnn_seq2seq_' + os.path.basename(
