# %load d_shared_code_masked_skip_connection_seq2seq.py
import pathlib
import copy
import sys
...
from tensorflow.python.ops.rnn_cell_impl import LSTMStateTuple, GRUCell, LSTMCell
...
class RLSTMCell(tf.compat.v1.nn.rnn_cell.BasicLSTMCell):
    ...
    def __init__(self, num_units, forget_bias=1.0, state_is_tuple=True, activation=None, reuse=None, dense=None):
        ...
        super(tf.comat.v1.nn.rnn_cell.BasicLSTMCell, self).__init__(_reuse=reuse)
...
class RSGRUCell(tf.compat.v1.nn.rnn_cell.GRUCell):
...
def Model(_abnormal_data, _abnormal_label, _hidden_num, _elem_num, _file_name, _partition):
    ...
    with g.as_default():
        ...
        l1_regularizer = tf.compat.v1.layers.l1_regularizer(scale=0.005, scope=None)
        ...
        with tf.variable_scope('decoder') as vs:
            if decode_without_input:
                ...
                for k in range(ensemble_space):
                    ...
                    if k == 0:
                        inference_decoder = tfa.seq2seq.BasicDecoder(pure_dec_cell, inference_helper,
                                                                            shared_state, output_layer=projection_layer)
                    elif k == 1:
                        inference_decoder = tfa.seq2seq.BasicDecoder(residual_dec_cell, inference_helper,
                                                                            shared_state, output_layer=projection_layer)
                    else:
                        inference_decoder = tfa.seq2seq.BasicDecoder(dec_cell, inference_helper, shared_state,
                                                                            output_layer=projection_layer)

                    d_dec['dec_output_{0}'.format(k)], _, _ = tfa.seq2seq.dynamic_decode(inference_decoder,
        ...
        regularization_penalty = tf.compat.v1.layers.apply_regularization(l1_regularizer, [shared_state])
