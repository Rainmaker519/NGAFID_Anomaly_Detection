...
def is_log_det_shape_matches_input(log_det, input, value_ndims, name=None):
    ...
    if not is_tensor_object(log_det):
        log_det = tf.convert_to_tensor(value=log_det)
    ...
    with tf.compat.v1.name_scope(name or 'is_log_det_shape_matches_input'):
        ...
        return tf.cond(
            pred=is_ndims_matches,
            true_fn=lambda: tf.reduce_all(input_tensor=tf.equal(
            ...
            false_fn=lambda: tf.constant(False, dtype=tf.bool)
        )
...
class Scale(object):
    ...
    def __init__(self, pre_scale, epsilon):
        ...
        Args:
            ...
            epsilon: Small float number to avoid dividing by zero or taking
                logarithm of zero.
        """
        self._pre_scale = tf.convert_to_tensor(value=pre_scale)
    ...
    def __rdiv__(self, other):
        return self._div(tf.convert_to_tensor(value=other))

    def __rtruediv__(self, other):
        return self._div(tf.convert_to_tensor(value=other))

    def __rmul__(self, other):
        return self._mult(tf.convert_to_tensor(value=other))
