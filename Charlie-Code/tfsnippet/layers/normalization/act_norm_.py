    ...
    def _transform(self, x, compute_y, compute_log_det):
        ...
        if not self._initialized:
            ...
            with tf.compat.v1.name_scope('initialization'):
                x_mean, x_var = tf.nn.moments(x=x, axes=reduce_axis)
            ...
            with tf.compat.v1.name_scope('log_det'):
                ...
                if reduce_ndims1 > 0:
                    log_det = tf.reduce_sum(
                        input_tensor=log_det, axis=list(range(-reduce_ndims1, 0)))
                    ...
                    if isinstance(reduce_shape1, tuple):
                        ...
                        log_det *= tf.cast(
                            tf.reduce_prod(input_tensor=reduce_shape1),
                            dtype=log_det.dtype
                        )
                ...
                if reduce_ndims2 > 0:
                    log_det = tf.reduce_sum(
                        input_tensor=log_det, axis=list(range(-reduce_ndims2, 0)))
