...
class RecurrentDistribution(Distribution):
    ...
    def sample_step(self, a, t):
        ...
        input_q_n = tf.broadcast_to(input_q_n,
                                    [tf.shape(input=z_previous)[0], tf.shape(input=input_q_n)[0], input_q_n.shape[1]])
        ...
        mu_q = tf.broadcast_to(mu_q, tf.shape(input=temp))
        std_q = tf.broadcast_to(std_q, tf.shape(input=temp))
        z_n = temp + mu_q
    ...
    def log_prob_step(self, _, t):
        ...
        if len(given_n.shape) > 2:
            input_q_n = tf.broadcast_to(input_q_n,
                                        [tf.shape(input=given_n)[0], tf.shape(input=input_q_n)[0], input_q_n.shape[1]])
    ...
    def __init__(self, input_q, mean_q_mlp, std_q_mlp, z_dim, window_length=100, is_reparameterized=True,
        ...
        self.input_q = tf.transpose(a=input_q, perm=[1, 0, 2])
        self._dtype = input_q.dtype
        self._is_reparameterized = is_reparameterized
        self._is_continuous = True
        self.z_dim = z_dim
        self.window_length = window_length
        self.time_first_shape = tf.convert_to_tensor(value=[self.window_length, tf.shape(input=input_q)[0], self.z_dim])
    ...
    def sample(self, n_samples=1024, is_reparameterized=None, group_ndims=0, compute_density=False,
        ...
        with tf.compat.v1.name_scope(name=name, default_name='sample'):
            ...
            noise = tf.transpose(a=noise, perm=[1, 0, 2])  # window_length * n_samples * z_dim
            noise = tf.random.truncated_normal(tf.shape(input=noise))

            time_indices_shape = tf.convert_to_tensor(value=[n_samples, tf.shape(input=self.input_q)[1], self.z_dim])
            ...
            samples = tf.transpose(a=samples, perm=[1, 2, 0, 3])  # n_samples * batch_size * time_step *  z_dim
    ...
    def log_prob(self, given, group_ndims=0, name=None):
        with tf.compat.v1.name_scope(name=name, default_name='log_prob'):
            if len(given.shape) > 3:
                time_indices_shape = tf.convert_to_tensor(value=[tf.shape(input=given)[0], tf.shape(input=self.input_q)[1], self.z_dim])
                given = tf.transpose(a=given, perm=[2, 0, 1, 3])
            else:
                time_indices_shape = tf.convert_to_tensor(value=[tf.shape(input=self.input_q)[1], self.z_dim])
            ...
            if len(given.shape) > 3:
                log_prob = tf.transpose(a=log_prob, perm=[1, 2, 0, 3])
            else:
                log_prob = tf.transpose(a=log_prob, perm=[1, 0, 2])
