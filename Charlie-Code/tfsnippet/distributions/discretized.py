...
class DiscretizedLogistic(Distribution):
    ...
    def __init__(self, mean, log_scale, bin_size, min_val=None, max_val=None,
        ...
        """
        # check the arguments
        mean = tf.convert_to_tensor(value=mean)
        param_dtype = mean.dtype
        log_scale = tf.convert_to_tensor(value=log_scale)
        ...
        with tf.compat.v1.name_scope('DiscretizedLogistic.init'):
            batch_shape = tf.broadcast_dynamic_shape(tf.shape(input=mean),
                                                     tf.shape(input=log_scale))
    ...
    def sample(self, n_samples=None, group_ndims=0, is_reparameterized=None,
        ...
        with tf.compat.v1.name_scope(name, default_name='DiscretizedLogistic.sample'):
            ...
            inverse_logistic_cdf = maybe_check_numerics(
                tf.math.log(u) - tf.math.log(1. - u), 'inverse_logistic_cdf')
    ...
    def log_prob(self, given, group_ndims=0, name=None):
        given = tf.convert_to_tensor(value=given)

        with tf.compat.v1.name_scope('DiscretizedLogistic.log_prob', values=[given]):
            ...
            middle_bins_pdf = tf.math.log(tf.maximum(cdf_delta, self._epsilon))
