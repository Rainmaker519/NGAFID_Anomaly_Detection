...
class Mixture(Distribution):
    ...
    def __init__(self, categorical, components, is_reparameterized=False):
        ...
        if settings.enable_assertions:
            with tf.compat.v1.name_scope('Mixture.init'):
    ...
    def sample(self, n_samples=None, group_ndims=0, is_reparameterized=None,
        ...
        #######################################################################
        with tf.compat.v1.name_scope(name or 'Mixture.sample'):
            ...
            samples = tf.reduce_sum(
                input_tensor=mask * tf.stack(c_samples, axis=-self.value_ndims - 1),
    ...
    def log_prob(self, given, group_ndims=0, name=None):
        ...
        with tf.compat.v1.name_scope(name or 'Mixture.log_prob', values=[given]):
