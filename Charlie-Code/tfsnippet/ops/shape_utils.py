...
def prepend_dims(x, ndims=1, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='prepend_dims', values=[x]):
    ...
    with tf.compat.v1.name_scope(name, default_name='flatten', values=[x]):
        if ndims == 1:
            ...
            if None in front_shape or None in back_shape:
                dynamic_shape = tf.shape(input=x)
...
def broadcast_to_shape(x, shape, name=None):
    ...
    with tf.compat.v1.name_scope(name=name or 'broadcast_to_shape', values=ns_values):
        ...
        if isinstance(shape, tuple) and x_shape is not None:
            ...
            if axis_to_check:
                need_multiply_flags = []
                x_dynamic_shape = tf.shape(input=x)
                ...
                if not need_multiply_ones:
                    need_multiply_ones = \
                        tf.reduce_any(input_tensor=tf.stack(need_multiply_flags))
        ...
        if post_assert_shape:
            post_assert_op = tf.compat.v1.assert_equal(
                tf.reduce_all(input_tensor=tf.equal(tf.shape(input=t)[-tf.size(input=shape):], shape)),
                True,
                message=cannot_broadcast_msg
            )
...
def broadcast_concat(x, y, axis, name=None):
    ...
    def broadcast_axis(i, a, b, a_tile, b_tile, a_tensor, b_tensor):
        ...
        if a is None and b is None:
            ...
            assertions.append(tf.compat.v1.assert_equal(a, b, message=err_msg))
    ...
    with tf.compat.v1.name_scope(name, default_name='broadcast_concat', values=[x, y]):
...
def transpose_conv2d_axis(input, from_channels_last, to_channels_last,
    ...
    if transpose_axis is not None:
        ...
        input = tf.transpose(a=input, perm=sample_and_batch_axis + transpose_axis,
