...
def add_n_broadcast(tensors, name=None):
    ...
    """
    tensors = [tf.convert_to_tensor(value=t) for t in tensors]
    if not tensors:
...
def log_sum_exp(x, axis=None, keepdims=False, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='log_sum_exp', values=[x]):
        x_max_keepdims = tf.reduce_max(input_tensor=x, axis=axis, keepdims=True)
        if not keepdims:
            x_max = tf.squeeze(x_max_keepdims, axis=axis)
        else:
            x_max = x_max_keepdims
        sum_exp = tf.reduce_sum(input_tensor=tf.exp(x - x_max_keepdims), axis=axis,
                                keepdims=keepdims)
...
def log_mean_exp(x, axis=None, keepdims=False, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='log_mean_exp', values=[x]):
        ...
        if not keepdims:
            ...
            x_max = x_max_keepdims
        mean_exp = tf.reduce_mean(input_tensor=tf.exp(x - x_max_keepdims), axis=axis,
                                  keepdims=keepdims)
