...
def classification_accuracy(y_pred, y_true, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='classification_accuracy',
                       values=[y_pred, y_true]):
        return tf.reduce_mean(
            input_tensor=tf.cast(tf.equal(y_pred, y_true), dtype=tf.float32))
...
def softmax_classification_output(logits, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='softmax_classification_output',
                       values=[logits]):
        return tf.argmax(input=logits, axis=-1, output_type=tf.int32)
