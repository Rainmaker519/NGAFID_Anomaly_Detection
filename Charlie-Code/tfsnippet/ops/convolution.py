...
def space_to_depth(input, block_size, channels_last=True, name=None):
    ...
    with tf.compat.v1.name_scope(name or 'space_to_depth', values=[input]):
        ...
        output = tf.compat.v1.space_to_depth(input=output, block_size=block_size, data_format=data_format)
...
def depth_to_space(input, block_size, channels_last=True, name=None):
    ...
    with tf.compat.v1.name_scope(name or 'space_to_depth', values=[input]):
        output, s1, s2 = flatten_to_ndims(input, ndims=4)
        output = tf.compat.v1.depth_to_space(input=output, block_size=block_size, data_format=data_format)
        output = unflatten_from_ndims(output, s1, s2)
        return output
