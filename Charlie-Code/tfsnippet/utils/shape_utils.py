...
def get_batch_size(tensor, axis=0, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='get_batch_size', values=[tensor]):
        ...
        if batch_size is None:
            batch_size = tf.shape(input=tensor)[axis]
    return batch_size
...
def get_dimension_size(tensor, axis, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='get_dimension_size',
                       values=[tensor]):
        shape = get_static_shape(tensor)
        ...
        return tf.shape(input=tensor)[axis]
...
def is_shape_equal(x, y, name=None):
    ...
    if x_shape is not None and y_shape is not None:
        ...
        with tf.compat.v1.name_scope(name or 'is_shape_equal', values=[x, y]):
            x_shape = get_shape(x)
            y_shape = get_shape(y)
            return tf.reduce_all(input_tensor=[tf.equal(x_shape[a], y_shape[a])
    ...
    with tf.compat.v1.name_scope(name or 'is_shape_equal', values=[x, y]):
        ...
        y_shape = get_shape(y)
        return tf.cond(
            pred=tf.equal(tf.rank(x), tf.rank(y)),
            true_fn=lambda: tf.reduce_all(
