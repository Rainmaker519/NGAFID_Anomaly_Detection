...
def p_net(observed=None, n_z=None, is_initializing=False):
    ...
    def make_component(i):
        normal = spt.Normal(
            mean=tf.compat.v1.get_variable('mean_{}'.format(i), shape=[1, config.z_dim],
                                 dtype=tf.float32, trainable=True),
            logstd=tf.maximum(
                tf.compat.v1.get_variable('logstd_{}'.format(i), shape=[1, config.z_dim],
                                dtype=tf.float32, trainable=True),
                config.z_logstd_min
            )
        )
...
def main():
    ...
    with tf.compat.v1.name_scope('initialization'), \
        ...
        init_lb = tf.reduce_mean(input_tensor=init_chain.vi.lower_bound.elbo())
    ...
    with tf.compat.v1.name_scope('training'):
        ...
        vae_loss = tf.reduce_mean(input_tensor=train_chain.vi.training.sgvb())
    ...
    with tf.compat.v1.name_scope('testing'):
        ...
        test_nll = -tf.reduce_mean(input_tensor=test_chain.vi.evaluation.is_loglikelihood())
        test_lb = tf.reduce_mean(input_tensor=test_chain.vi.lower_bound.elbo())
