...
def gaussian_mixture_prior(y, z_dim, n_clusters):
    ...
    z_mean = tf.nn.embedding_lookup(params=prior_mean, ids=y)
    ...
    if config.p_z_given_y_std == 'one':
        ...
        z_std_or_logstd = tf.nn.embedding_lookup(params=prior_std_or_logstd, ids=y)
...
def main():
    ...
    with tf.compat.v1.name_scope('training'):
        ...
        if config.vi_algorithm == 'reinforce':
            ...
            vae_loss = tf.reduce_mean(
                input_tensor=train_chain.vi.training.reinforce(baseline=baseline))
        else:
            assert(config.vi_algorithm == 'vimco')
            vae_loss = tf.reduce_mean(input_tensor=train_chain.vi.training.vimco())
    ...
    with tf.compat.v1.name_scope('testing'):
        ...
        test_nll = -tf.reduce_mean(
            input_tensor=test_chain.vi.evaluation.is_loglikelihood())

        # derive the classifier via q(y|x)
        q_y_given_x = tf.argmax(input=test_q_net['y'].distribution.logits,
    ...
    with tf.compat.v1.name_scope('plotting'):
        ...
        x_plots = tf.reshape(
            tf.transpose(a=bernoulli_as_pixel(plot_p_net['x']), perm=(1, 0, 2)),
            (-1, 28, 28)
        )
    ...
    with spt.utils.create_session().as_default() as session, \
        ...
        with spt.TrainLoop(params,
            ...
            trainer = spt.Trainer(
                ...
                metrics={'loss': loss},
                summaries=tf.compat.v1.summary.merge_all(spt.GraphKeys.AUTO_HISTOGRAM)
            )
