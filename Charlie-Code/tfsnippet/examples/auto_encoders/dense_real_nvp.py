...
def coupling_layer_shift_and_scale(x1, n2):
    ...
    shift = spt.layers.dense(
        ...
        bias_initializer=tf.compat.v1.zeros_initializer(), scope='shift'
    )
    scale = spt.layers.dense(
        h, n2, kernel_initializer=tf.compat.v1.zeros_initializer(),
        bias_initializer=tf.compat.v1.zeros_initializer(), scope='scale'
    )
    return shift, scale
...
def main():
    ...
    with tf.compat.v1.name_scope('initialization'), \
        ...
        init_loss = tf.reduce_mean(input_tensor=init_chain.vi.training.sgvb())
