...
def elbo_objective(log_joint, latent_log_prob, axis=None, keepdims=False,
    ...
    """
    log_joint = tf.convert_to_tensor(value=log_joint)
    latent_log_prob = tf.convert_to_tensor(value=latent_log_prob)
    with tf.compat.v1.name_scope(name,
                       default_name='elbo_objective',
                       values=[log_joint, latent_log_prob]):
        objective = log_joint - latent_log_prob
        if axis is not None:
            objective = tf.reduce_mean(input_tensor=objective, axis=axis, keepdims=keepdims)
        return objective
