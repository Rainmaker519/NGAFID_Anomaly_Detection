...
def sgvb_estimator(values, axis=None, keepdims=False, name=None):
    ...
    with tf.compat.v1.name_scope(name, default_name='sgvb_estimator', values=[values]):
        estimator = values
        if axis is not None:
            estimator = tf.reduce_mean(input_tensor=estimator, axis=axis, keepdims=keepdims)
        return estimator
...
def iwae_estimator(log_values, axis, keepdims=False, name=None):
    ...
    log_values = tf.convert_to_tensor(value=log_values)
...
def nvil_estimator(values, latent_log_joint, baseline=None,
    ...
    values = tf.convert_to_tensor(value=values)  # f(x,z)
    latent_log_joint = tf.convert_to_tensor(value=latent_log_joint)  # log q(z|x)
    if baseline is not None:
        baseline = tf.convert_to_tensor(value=baseline)
    dtype = values.dtype

    @contextmanager
    def mk_scope():
        if center_by_moving_average:
            with tf.compat.v1.variable_scope(None, default_name=name or 'nvil_estimator'):
                yield
        else:
            ns_values = [values, latent_log_joint]
            if baseline is not None:
                ns_values += [baseline]
            with tf.compat.v1.name_scope(name or 'nvil_estimator', values=ns_values):
                yield
    ...
    with mk_scope():
        ...
        if baseline is not None:
            ...
            with tf.compat.v1.name_scope('baseline_cost'):
                baseline_cost = tf.square(
                    tf.stop_gradient(l_signal) - baseline)
                if axis is not None:
                    baseline_cost = tf.reduce_mean(
                        input_tensor=baseline_cost, axis=axis, keepdims=keepdims)

            l_signal = l_signal - baseline

        # estimate `c` by moving average
        if center_by_moving_average:
            with tf.compat.v1.name_scope('center_by_moving_average'):
                batch_center = tf.reduce_mean(
                    input_tensor=l_signal, axis=batch_axis, keepdims=True)
        ...
        with tf.compat.v1.name_scope('cost'):
            ...
            if axis is not None:
                cost = tf.reduce_mean(input_tensor=cost, axis=axis, keepdims=keepdims)
...
def _vimco_control_variate(log_f, axis):
    ...
    mean_except_k = (
        (tf.reduce_mean(input_tensor=log_f, axis=axis, keepdims=True) - log_f / K_f) *
        (K_f / (K_f - 1))
    )
...
\\end{aligned}
    ...
    log_values = tf.convert_to_tensor(value=log_values)  # log f(x,z)
    latent_log_joint = tf.convert_to_tensor(value=latent_log_joint)  # log q(z|x)

    with tf.compat.v1.name_scope(name, default_name='vimco_estimator',
        ...
        fake_term = tf.reduce_sum(
            input_tensor=latent_log_joint * tf.stop_gradient(true_term - control_variate),
            axis=axis,
            keepdims=keepdims
        )
        if not keepdims:
