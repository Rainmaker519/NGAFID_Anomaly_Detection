...
def collect_outputs(outputs, inputs, data_flow, mode='concat', axis=0,
    ...
    if isinstance(outputs, (dict, OrderedDict)):
        ...
        outputs = [tf.convert_to_tensor(value=outputs[k]) for k in output_keys]
    else:
        output_keys = None
        outputs = [tf.convert_to_tensor(value=o) for o in outputs]
    inputs = [tf.convert_to_tensor(value=i) for i in inputs]
