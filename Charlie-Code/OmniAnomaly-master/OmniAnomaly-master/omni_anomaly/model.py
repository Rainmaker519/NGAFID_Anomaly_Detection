...
class OmniAnomaly(VarScopeObject):
        ...
        with reopen_variable_scope(self.variable_scope):
            ...
            self._vae = VAE(
                ...
                q_z_given_x=partial(RecurrentDistribution,
                                    mean_q_mlp=partial(tf.compat.v1.layers.dense, units=config.z_dim, name='z_mean', reuse=tf.compat.v1.AUTO_REUSE),
                ...
                h_for_p_x=Lambda(
                    partial(
                        ...
                        h_for_dist=lambda x: rnn(x=x,
                                                 ...
                                                 name='rnn_p_x'),
                        mean_layer=partial(
                            tf.compat.v1.layers.dense, units=config.x_dim, name='x_mean', reuse=tf.compat.v1.AUTO_REUSE
                        ),
                        std_layer=partial(
                ...
                h_for_q_z=Lambda(
                    ...
                    partial(
                        ...
                        h_for_dist=lambda x: rnn(x=x,
                                                 ...
                                                 name="rnn_q_z"),
                        mean_layer=partial(
                            tf.compat.v1.layers.dense, units=config.z_dim, name='z_mean', reuse=tf.compat.v1.AUTO_REUSE
                        ),
                        std_layer=partial(
    ...
    def get_training_loss(self, x, n_z=None):
        ...
        with tf.compat.v1.name_scope('training_loss'):
            ...
            log_joint = tf.reduce_sum(input_tensor=x_log_prob, axis=-1)
            ...
            loss = tf.reduce_mean(input_tensor=vi.training.sgvb())
            return loss
    ...
    def get_score(self, x, n_z=None,
        ...
        with tf.compat.v1.name_scope('get_score'):
            ...
            z_mean = tf.reduce_mean(input_tensor=z_samples, axis=0) if n_z is not None else z_samples
            z_std = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(z_samples - z_mean), axis=0) / (n_z - 1)) \
